...
[Fact]
public async Task Logout_ShouldReturnBadRequest_WhenTokenWasAlreadyRevoked()
{
    // Arrange
    await _factory.ResetDatabaseAsync();
    ...
    Assert.Equal(
        "Der Refresh Token ist ungültig oder bereits widerrufen.",
        result.Message);
}

[Fact]
public async Task Logout_ShouldReturnBadRequest_WhenTokenIsUnknown()
{
    // Arrange
    await _factory.ResetDatabaseAsync();
